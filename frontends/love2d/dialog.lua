...
--- Open a machine ROM / cart / firmware file.
-- opts (optional): title, prompt, filter_win, filter_zenity, mac_types,
--                  default_ext, default_dir
function M.choose_open_rom(opts)
  opts = opts or {}
  local title = (opts.title or "Load ROM"):gsub('"', "")
  local prompt = (opts.prompt or title):gsub('"', "")
  local filter_win = opts.filter_win
    or "ROM dumps (*.rom;*.bin)|*.rom;*.bin|All files (*.*)|*.*"
  local filter_zenity = opts.filter_zenity or "ROM dumps | *.rom *.bin"
  local mac_types = opts.mac_types or '{"rom","bin","public.data"}'
  local default_ext = opts.default_ext or "rom"
  local default_dir = opts.default_dir and opts.default_dir:gsub('"', "") or nil
