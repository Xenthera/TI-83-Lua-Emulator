-- Z80 CPU interpreter for TI-83 Plus.
...
function Z80.new(bus)
  ...
  self.bus = bus -- { read, write, in_port, out_port, irq_pending, ack_irq }
  self:reset()
  return self
end
...
function Z80:accept_interrupt()
  ...
  if self.im == 0 or self.im == 1 then
    ...
    self:push16(self.pc)
    local vec = pair(self.i, 0xFF) -- data bus often 0xFF on TI
...
local opcodes = require("core.cpu.opcodes")
