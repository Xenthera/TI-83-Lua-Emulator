#!/usr/bin/env lua
-- Build a single-file ComputerCraft export of the TI-89 Titanium emulator.
--
-- Prefer (Windows):
--   tools\export_computercraft_ti89.cmd
-- Or:
...
local entry = "frontends.computercraft.api_ti89"
...
-- Run the bundler in-process (no nested cmd quoting / PATH shims).
local saved_arg = arg
arg = {
  [-1] = saved_arg[-1],
  [0] = bundle_path,
...
io.stdout:write("  1) ti89_cc.lua   (from dist/ti89_cc.lua)\n")
io.stdout:write("  2) ti89.rom or ti89titanium.rom\n")
io.stdout:write("  3) one+ advanced monitors (LCD needs ~80x34 @ scale 0.5, sixtel)\n")
io.stdout:write("Then run:\n")
io.stdout:write("  ti89_cc --help\n")
io.stdout:write("  ti89_cc --rom ti89titanium.rom\n")
