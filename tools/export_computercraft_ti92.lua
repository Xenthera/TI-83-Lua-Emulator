#!/usr/bin/env lua
-- Build a single-file ComputerCraft export of the TI-92 Plus emulator.
--
-- Prefer (Windows):
--   tools\export_computercraft_ti92.cmd
-- Or:
--   tools\luajit\luajit.exe tools/export_computercraft_ti92.lua

local ROOT = arg[0]:match("(.+)[/\\]tools[/\\]export_computercraft_ti92%.lua$") or "."

local out = "dist/ti92_cc.lua"
local i = 1
while i <= #arg do
  if arg[i] == "-o" then
    i = i + 1
...
local entry = "frontends.computercraft.api_ti92"
