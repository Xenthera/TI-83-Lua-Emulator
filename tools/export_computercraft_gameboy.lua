#!/usr/bin/env lua
-- Build a single-file ComputerCraft export of the Game Boy (DMG) emulator.
--
-- Prefer (Windows):
--   tools\export_computercraft_gameboy.cmd
-- Or:
--   tools\luajit\luajit.exe tools/export_computercraft_gameboy.lua

local ROOT = arg[0]:match("(.+)[/\\]tools[/\\]export_computercraft_gameboy%.lua$") or "."

local out = "dist/gb_cc.lua"
local i = 1
while i <= #arg do
  if arg[i] == "-o" then
    i = i + 1
...
local entry = "frontends.computercraft.api_gameboy"
...
io.stdout:write("  1) gb_cc.lua   (from dist/gb_cc.lua)\n")
...
io.stdout:write("  gb_cc --help\n")
io.stdout:write("  gb_cc --rom tetris.gb\n")
