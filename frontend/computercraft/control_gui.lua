...
function M.next_role(role)
  if role == "lcd" then return "pad" end
  if role == "pad" then return "mem" end
  if role == "mem" then return "off" end
  return "lcd"
end
...
--- List .8xk / .8xp / .8xg in dir (non-recursive).
function M.scan_files(dir)
  ...
  for _, name in ipairs(names) do
    ...
    local kind = nil
    if lower:match("%.8xk$") then
      kind = "app"
    elseif lower:match("%.8xg$") then
      kind = "group"
    elseif lower:match("%.8xp$") then
      kind = "prog"
    end
    if kind then
      local path = name
  ...
  local kind_order = { app = 1, group = 2, prog = 3 }
...
function M.new(opts)
  ...
  self.files = M.scan_files(self.dir)
  self.scroll = 0
  self.perf_mhz = 0
  self.perf_pct = 0
...
function M:draw_status()
  ...
  local function btn(label, bg, fg, action, payload)
    ...
    if x + #label - 1 > w then
      label = label:sub(1, math.max(1, w - x + 1))
    end
  ...
  local room = w - x - 4 -- leave a little room for theme / edge
  if room < fw then
    fw = math.max(3, room)
  end
  ...
  if fw >= 3 and x + fw - 1 <= w then
    ...
    x = x + fw
    -- trailing spacer so theme doesn't jam against the field
    if x <= w then
  ...
  local right_x = w - #right - quit_w + 1
  if right_x < x + 1 then
    right = string.format("%.1fM ", self.perf_mhz)
    right_x = w - #right - quit_w + 1
  end
  if right_x < x + 1 then
    right = ""
    right_x = w - quit_w + 1
  end
  if right ~= "" and right_x >= 1 then
...
function M:draw()
  ...
  if #self.files == 0 then
    fill_row(self.y_files0, w, C.black, C.gray, " (no .8xk / .8xp / .8xg in this folder)")
  else
    for row = 0, self.list_h - 1 do
      ...
      if not f then
        ...
        local tag, tag_bg
        if f.kind == "app" then
          tag, tag_bg = "APP ", C.yellow
        elseif f.kind == "group" then
          tag, tag_bg = "GRP ", C.pink
        else
          tag, tag_bg = "PRGM", C.lime
        end
        local name = f.name
        local room = w - 7
        if #name > room then
