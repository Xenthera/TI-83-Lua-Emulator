...
--- Map style -> { face_bg, face_fg } using CC palette.
...
function M:draw()
  ...
  local mon = self.mon
  local mw, mh = mon.getSize()
  local body = C.gray -- faceplate below the F-key header
  local fkey_bg = C.black
  local alpha_fg = C.lightBlue or C.cyan
  ...
  for _, b in ipairs(self.buttons) do
    ...
    local function paint_legends(y, bg)
      ...
      -- Top graph row: yellow 2nd name left, lightBlue F1-F5 on the right.
      if b.fkey then
        local fk = tostring(b.fkey)
        local fkey_fg = C.lightBlue or C.cyan
        if b.second then
          local left = tostring(b.second)
          local room = math.max(1, width - #fk - 1)
          left = left:sub(1, room)
          write_clipped(mon, lx0, y, left, C.yellow, bg, lx0, lx1)
      ...
      elseif b.second then
        write_clipped(mon, lx0, y, b.second, C.yellow, bg, lx0, lx1)
      elseif b.alpha then
        local right = tostring(b.alpha)
    ...
    if b.legend_on_face and (b.fkey or b.second or b.alpha) and face_h == 1 then
      if b.fkey then
        ...
        if left ~= "" then
          ...
          left = left:sub(1, room)
          write_clipped(mon, b.x0, b.face_y0, left, C.yellow, face_bg, b.x0, b.x1)
        end
